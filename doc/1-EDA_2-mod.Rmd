...
When rendering, set `FASTER` to `TRUE` to omit heavier calculations (with them, it can take more than 15 minutes to produce HTML).
...
* zoo: mainly for na.omit/na.fill functions
...
library(forcats); library(purrr); library(magrittr); library(reshape2); library(haven)
experiment_data <-
  haven::read_sav(system.file(package = 'experRrvey', 'extdata/experiment_data.sav'))
survey_data <-
  haven::read_sav(system.file(package = 'experRrvey', 'extdata/survey_data.sav'))
...
# just to view other attributes present in data:
...
is_q <- purrr::map_lgl(survey_data, ~ inherits(., c("haven_labelled", "double")))
...
```{r some_data_stats}
survey_data %>% 
 select(hours, d_h_hnumber, d_parent, s_problem, s_hhincome) %T>%
 {print(summary(.))} %>% 
...
```{r likert_plots}
library(likert); library(ggplot2); library(ggstance)
...
```{r all_combos, eval=(if (exists('FASTER')) !FASTER else TRUE), out.width=c('50%', '50%'), fig.show='hold'}
...
survey_data[,'r_id_intnsty'] <- survey_data %>% select(-weights) %>% 
...
survey_data_all_intnsts <- survey_data %>% select(-weights, -r_id_intnsty) %>%
...
ggplot(survey_data[,c('response_id', 'r_id_intnsty')], aes(y = r_id_intnsty, x = response_id)) +
  geom_point() + theme(axis.title = element_blank(), axis.text.x=element_blank(), 
...
```{r boxplots, eval=(if (exists('FASTER')) !FASTER else TRUE)}
...
experiment_data %>% select(-response_id) %>% group_by(answer) %>%
...
sel.smpl.cnjnt.rad$model_list$full$coeff[,
                    c('label', 'coefficient', 'p.value')] %>% kable()
# bug with significance replaced with NAs
```
...
m1.oos <- predict(m1, prd_sample[,-which(names(prd_sample) == 'answer')])$fit %>%
  apply(MARGIN = 1, FUN = function(x) which(x == max(x)))
...
ggplot(melt(m1.m2.m3.oos, id = 'obs', variable.name = 'model'),
       aes(x = obs, y = value, color = model)) + 
  geom_point(shape = 3, position = ggstance::position_dodgev(height=0.3))
